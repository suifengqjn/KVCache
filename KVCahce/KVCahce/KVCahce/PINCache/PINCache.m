...
#pragma mark - PINCacheExposedMethods

- (void)setRaw:(NSData *)object forKey:(NSString *)key block:(PINCacheObjectBlock)block
{
    if (!key || !object)
        return;
    ...
    [_memoryCache setObject:object forKey:key block:memBlock];
    [_diskCache setRaw:object forKey:key block:diskBlock];
...
- (void)rawForKey:(NSString *)key block:(PINCacheObjectBlock)block
{
    if (!key || !block)
        return;
    ...
    dispatch_async(_concurrentQueue, ^{
        ...
        [strongSelf->_memoryCache objectForKey:key block:^(PINMemoryCache *cache, NSString *key, id object) {
            ...
            if (object) {
                ...
                [strongSelf->_diskCache rawForKey:key block:^(PINDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
                    PINCache *strongSelf = weakSelf;
                    if (!strongSelf)
                        return;
...
- (void)setRaw:(NSData *)object forKey:(NSString *)key
{
    ...
    [self setRaw:object forKey:key block:^(PINCache *cache, NSString *key, id object) {
...
- (NSData *)rawForKey:(NSString *)key
{
    ...
    [self rawForKey:key block:^(PINCache *cache, NSString *key, id object) {
        objectForKey = object;
        dispatch_semaphore_signal(semaphore);
    }];
