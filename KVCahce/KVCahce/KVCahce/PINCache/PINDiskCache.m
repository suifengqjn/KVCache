...
#pragma mark - PINDiskCacheExposedMethods

- (void)setRaw:(NSData *)object forKey:(NSString *)key block:(PINDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
    ...
    dispatch_async(_asyncQueue, ^{
        ...
        // 先保存成临时文件，成功后再改名
        BOOL written = [fileManager createFileAtPath:[[fileURL path] stringByAppendingString:@".tmp"] contents:object attributes:nil];
        if (written) {
            
            // 如果已经有原文件则先删除再改名
            [fileManager removeItemAtPath:[fileURL path] error:nil];
            BOOL moved = [fileManager moveItemAtPath:[[fileURL path] stringByAppendingString:@".tmp"] toPath:[fileURL path] error:nil];
            if (moved) {
                ...
                fileURL = nil;
                [[NSNotificationCenter defaultCenter] postNotificationName:KVCacheErrorReport
                                                                    object:[NSString stringWithFormat:@"%@ 文件缓存错误2", NSStringFromClass([self class])]];
            }
            
        } else {
            fileURL = nil;
            [[NSNotificationCenter defaultCenter] postNotificationName:KVCacheErrorReport
                                                                object:[NSString stringWithFormat:@"%@ 文件缓存错误1", NSStringFromClass([self class])]];
        }
...
- (void)rawForKey:(NSString *)key block:(PINDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)setRaw:(NSData *)object forKey:(NSString *)key
{
    if (!object || !key)
        return;
    ...
    [self setRaw:object forKey:key block:^(PINDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
...
- (NSData *)rawForKey:(NSString *)key
{
    if (!key)
        return nil;
    
    __block NSData *objectForKey = nil;
    
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    
    [self rawForKey:key block:^(PINDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        objectForKey = (NSData *)object;
        dispatch_semaphore_signal(semaphore);
    }];
